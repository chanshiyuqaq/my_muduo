//文件作用：
//统一封装：IPv4 IPv6 根据参数决定初始化哪种协议
//自动处理字节序转换 Endian
//IP地址与字符 相互转换
//确保这个类可以直接强转为 底层Socket API (bind,connect)
//域名解析：resolve
//特殊处理：INADDR_ANY 监听本机所有网卡 | INADDR_LOOPBACK 只监听回环地址
...
//确保：确保 InetAddress 这个 C++ 类在内存布局上，与 Linux 系统底层的 sockaddr_in 和 sockaddr_in6 结构体完全对齐且兼容
